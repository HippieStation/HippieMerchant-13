...
/obj/machinery/plantgenes/RefreshParts() // Comments represent the max you can set per tier, respectively. seeds.dm [219] clamps these for us but we don't want to mislead the viewer.
	for(var/obj/item/stock_parts/manipulator/M in component_parts)
		if(M.rating > 3)
			...
			max_potency = initial(max_potency) + (M.rating * 15) // old version: 53,59,77,95 	 Clamps at 100, new version: 50, 65, 80, 100

		max_yield = initial(max_yield) // old version: 4,6,8,10 	Clamps at 10, new version: 10 at all levels

	for(var/obj/item/stock_parts/scanning_module/SM in component_parts)
		if(SM.rating > 3) //If you create t5 parts I'm a step ahead mwahahaha!
			min_production = 1
		else
			min_production = 1 // old version: min_production = 12 - (SM.rating * 3)

		max_endurance = initial(max_endurance) + (SM.rating * 15) // old version: 35,60,85,100	Clamps at 10min 100max, new version: 55, 70, 85, 100
