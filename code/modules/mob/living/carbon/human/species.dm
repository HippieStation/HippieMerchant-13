...
/datum/species/proc/harm(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/martial_art/attacker_style)
	...
	if(attacker_style?.harm_act(user,target) == MARTIAL_ATTACK_SUCCESS)
		...
		if(targeted_zone == BODY_ZONE_PRECISE_GROIN) //Ball punching gives target bad mood, but deal 0.75x damage.
			target.visible_message(span_danger("[user] [atk_verb] [target]'s penis!"), \
							span_userdanger("Your penis got [atk_verb]ed by [user]!"))
			to_chat(user, span_danger("You [atk_verb] [target]'s penis!"))
			if(target.stat != DEAD)
