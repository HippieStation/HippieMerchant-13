...
/datum/reality_smash_tracker
	...
	var/list/obj/effect/eldritch_influence/smashes = list()
...
/datum/reality_smash_tracker/proc/add_to_smashes(datum/mind/to_add)
	for(var/obj/effect/eldritch_influence/reality_smash as anything in smashes)
...
/datum/reality_smash_tracker/proc/generate_new_influences()
	...
	while((length(smashes) + num_drained) < how_many_can_we_make && location_sanity < 100)
		...
		var/obj/effect/eldritch_influence/what_if_i_have_one = locate() in nearby_things
		var/obj/effect/visible_eldritch_influence/what_if_i_had_one_but_its_used = locate() in nearby_things
...
/obj/effect/visible_eldritch_influence
...
/obj/effect/visible_eldritch_influence/Initialize(mapload)
	. = ..()
 ...
 */
/obj/effect/visible_eldritch_influence/proc/show_presence()
	animate(src, alpha = 255, time = 15 SECONDS)

/obj/effect/visible_eldritch_influence/attack_hand(mob/living/user, list/modifiers)
	. = ..()
	if(.)
		return
	if(!ishuman(user))
...
/obj/effect/visible_eldritch_influence/attack_tk(mob/user)
	if(!ishuman(user))
		return
...
/obj/effect/visible_eldritch_influence/examine(mob/user)
	. = ..()
...
/obj/effect/eldritch_influence/Initialize(mapload)
	. = ..()
...
/obj/effect/eldritch_influence/attack_hand_secondary(mob/user, list/modifiers)
...
/obj/effect/eldritch_influence/attackby(obj/item/weapon, mob/user, params)
	. = ..()
	if(.)
		return
 ...
 */
/obj/effect/eldritch_influence/proc/drain_influence(mob/living/user, knowledge_to_gain)
 ...
 */
/obj/effect/eldritch_influence/proc/after_drain(mob/living/user)
	if(user)
		to_chat(user, span_hypnophrase(pick(strings(ELDRITCH_INFLUENCE_FILE, "drain_message"))))
		to_chat(user, span_warning("[src] begins to fade into reality!"))

	var/obj/effect/visible_eldritch_influence/illusion = new /obj/effect/visible_eldritch_influence(drop_location())
	illusion.name = "\improper" + pick(strings(ELDRITCH_INFLUENCE_FILE, "drained")) + " " + format_text(name)
 ...
 */
/obj/effect/eldritch_influence/proc/on_examine(atom/source, mob/user, list/examine_list)
	SIGNAL_HANDLER
 ...
 */
/obj/effect/eldritch_influence/proc/add_mind(datum/mind/heretic)
	minds |= heretic
 ...
 */
/obj/effect/eldritch_influence/proc/remove_mind(datum/mind/heretic)
	if(!(heretic in minds))
 ...
 */
/obj/effect/eldritch_influence/proc/generate_name()
	name = "\improper" + pick(strings(ELDRITCH_INFLUENCE_FILE, "prefix")) + " " + pick(strings(ELDRITCH_INFLUENCE_FILE, "postfix"))
