...
/datum/antagonist/heretic/ui_data(mob/user)
	...
	for(var/datum/eldritch_knowledge/knowledge as anything in get_researchable_knowledge())
		...
		if(ispath(knowledge, /datum/eldritch_knowledge/final))
...
/datum/antagonist/heretic/ui_act(action, params)
	...
	switch(action)
		if("research")
			var/datum/eldritch_knowledge/researched_path = text2path(params["path"])
...
/datum/antagonist/heretic/proc/passive_influence_gain()
	...
	if(owner.current.stat <= SOFT_CRIT)
		to_chat(owner.current, "[span_hear("You hear a whisper...")] [span_hypnophrase(pick(strings(ELDRITCH_INFLUENCE_FILE, "drain_message")))]")
...
/datum/antagonist/heretic/proc/admin_change_points(mob/admin)
	...
	var/change_num = input(admin, "Add knowledge points", "Points", 0) as num|null
...
/datum/antagonist/heretic/antag_panel_data()
	...
	for(var/knowledge_index in researched_knowledge)
		var/datum/eldritch_knowledge/knowledge = researched_knowledge[knowledge_index]
		if(istype(knowledge, /datum/eldritch_knowledge/final))
 ...
 */
/datum/antagonist/heretic/proc/gain_knowledge(datum/eldritch_knowledge/knowledge_type)
