 ...
 */
/datum/eldritch_knowledge/limited_amount/starting/base_void
...
/datum/eldritch_knowledge/limited_amount/starting/base_void/on_research(mob/user)
	. = ..()
	var/datum/antagonist/heretic/our_heretic = IS_HERETIC(user)
	our_heretic.heretic_path = route

/datum/eldritch_knowledge/limited_amount/starting/base_void/recipe_snowflake_check(mob/living/user, list/atoms, list/selected_atoms, turf/loc)
	if(!isopenturf(loc))
		user.balloon_alert(user, "ritual failed, invalid location!")
		return FALSE

	var/turf/open/our_turf = loc
	if(our_turf.GetTemperature() > T0C)
		user.balloon_alert(user, "ritual failed, not cold enough!")
		return FALSE
...
/datum/eldritch_knowledge/blade_upgrade/void
	...
	next_knowledge = list(/datum/eldritch_knowledge/spell/void_pull)
	route = PATH_VOID
...
/datum/eldritch_knowledge/final/void_final/recipe_snowflake_check(mob/living/user, list/atoms, list/selected_atoms, turf/loc)
	if(!isopenturf(loc))
		user.balloon_alert(user, "ritual failed, invalid location!")
		return FALSE
