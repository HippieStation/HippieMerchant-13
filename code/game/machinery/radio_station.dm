...
/obj/machinery/radio_station/attack_hand(mob/living/user)
	...
	switch(music_name)
		...
		if("The Last Stand - Sabaton")
			music_file = 'sound/radio_songs/TheLastStand.ogg'
...
/obj/machinery/radio_station/proc/playMusicToRadios(mob/living/user)
	...
	can_eject_disk = 1
	if(NOPOWER || BROKEN) //Need to check again in case the radio station is destroyed while this proc is in progress
		update_icon()
		stopRadioMusic()
		return
